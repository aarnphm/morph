...
export function ReasoningPanel({
  ...
  const memoizedCheckIcon = useMemo(() => <CheckIcon className="h-3 w-3 text-green-500" />, [])
  ...
  return (
    <div className={cn("w-full", className)}>
      ...
      {isExpanded && reasoning && (
        <div
          ref={reasoningRef}
          className="text-xs text-muted-foreground whitespace-pre-wrap ml-2 p-2 border-l-2 border-muted overflow-y-auto scrollbar-hidden max-h-72 transition-all duration-200 animate-in slide-in-from-top-2 duration-300 ease-in-out"
        >
          <span>{reasoning}</span>
        </div>
      )}
