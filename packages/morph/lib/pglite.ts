...
export async function hasNoteEmbedding(noteId: string): Promise<boolean> {
  try {
    ...
    const result = await pg.query(
      `SELECT 1 FROM ${tableName} WHERE note_id = $1 LIMIT 1`,
      [noteId]
    );
...
export async function saveEssayEmbedding(vaultId: string, fileId: string, chunkId: string, embedding: number[]) {
  try {
    const metadata = await fetchEmbedMetadata();
    ...
    console.debug(`Embedding saved for essay chunk ${vaultId}/${fileId}/${chunkId} in table ${tableName}`);
  } catch (error) {
    console.error(`Failed to save embedding for essay chunk ${vaultId}/${fileId}/${chunkId}:`, error);
    throw error;
  }
}

// Function to check if an essay embedding exists for a specific chunk
export async function hasEssayEmbedding(vaultId: string, fileId: string, chunkId: string): Promise<boolean> {
  try {
    const metadata = await fetchEmbedMetadata();
    const tableName = getEssayEmbeddingTableName(metadata);
    const pg = await getPgLiteInstance();
    ...
    console.error(`Failed to check embedding for essay chunk ${vaultId}/${fileId}/${chunkId}:`, error);
    return false;
  }
}
...
export async function findSimilarNotes(queryEmbedding: number[], limit: number = 5): Promise<string[]> {
  try {
    const metadata = await fetchEmbedMetadata();
    const tableName = getNoteEmbeddingTableName(metadata);
    const pg = await getPgLiteInstance();
...
export async function findSimilarEssays(vaultId: string, queryEmbedding: number[], limit: number = 5): Promise<string[]> {
  try {
    const metadata = await fetchEmbedMetadata();
    const tableName = getEssayEmbeddingTableName(metadata);
    const pg = await getPgLiteInstance();
...
export async function getEssayEmbedding(vaultId: string, fileId: string, chunkId: string): Promise<number[] | null> {
  try {
    const metadata = await fetchEmbedMetadata();
    const tableName = getEssayEmbeddingTableName(metadata);
    const pg = await getPgLiteInstance();
